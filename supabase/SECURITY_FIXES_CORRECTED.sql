...
-- ==================== DROP DEPENDENT VIEWS (Required for column alterations) ====================
DROP VIEW IF EXISTS leaderboard CASCADE;
DROP VIEW IF EXISTS truth_hunters_leaderboard CASCADE;
DROP VIEW IF EXISTS squad_leaderboard CASCADE;
...
-- 2.1 Add length constraints to text fields (prevents DoS via huge inputs)
ALTER TABLE profiles
ALTER COLUMN display_name TYPE VARCHAR(100);

ALTER TABLE submissions
ALTER COLUMN context_description TYPE VARCHAR(2000),
ALTER COLUMN claimed_context TYPE VARCHAR(500),
ALTER COLUMN source_url TYPE VARCHAR(2000);

ALTER TABLE squads
ALTER COLUMN name TYPE VARCHAR(50),
ALTER COLUMN description TYPE VARCHAR(500);

-- 2.2 Add check constraints for URLs
ALTER TABLE submissions
